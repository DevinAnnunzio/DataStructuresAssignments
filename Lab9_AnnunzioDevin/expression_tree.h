...
//expression_tree& operator = (const expression_tree& other);
  ...
  int evaluate(const binary_tree_node<std::string>* n) const;
...
//precondition: input value should be a valid string containing only digits
//postcondition: returns the string value converted to a valid integer
